...
#### Plot ####
plot1 <- ggplot(data_bar_melt, aes(disease, y = value*100, fill = disease, color = variable)) +
  geom_bar(stat = "identity", position="dodge", width = 0.8, size = 1) +
  annotate("text", x = 6, y = 60, label = "Bold Border denotes Percent Reduction\nper Day of Quarantine", col = "black", size = 2.5) +
  scale_fill_manual(values = my.cols) +
  scale_color_manual(values = c(NA, "black")) +
  ylab(expression(paste("Percent reduction in", R[e], " by Quarantine over Symptom Monitoring"))) +
  xlab("Disease") +
  theme_bw() +  guides(color=FALSE)
plot1

date <- format(Sys.time(), "%Y%m%d")
pdf(file=paste("~/Dropbox/Ebola/General_Quarantine_Paper/General_Quarantine_Paper/", date, "_RelBenefitBars.pdf", sep=""))
plot(plot1)
dev.off()
