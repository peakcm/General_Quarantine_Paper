#### Header #### 
# MERS Case Study for Generalized Study of Quarantine vs Symptom Monitoring
# Corey Peak
# Version 1.0
# August 25, 2015
...
parms_serial_interval <- list("lognormal", 7.6, 1.77) #Assiri
names(parms_serial_interval) <- c("dist","parm1","parm2")

parms_T_inc = list("lognormal", 5.2, 1.7, 999, "independent", "independent")  #Assiri
...
# High Performers
layout(cbind(c(1),c(2),c(3),c(4)))
plot(data[data$ks < 0.1, "T_lat_offset"], data[data$ks < 0.1, "ks"] )
plot(data[data$ks < 0.1, "d_symp_lower"], data[data$ks < 0.1, "ks"])
plot(data[data$ks < 0.1, "d_symp_upper"], data[data$ks < 0.1, "ks"])
plot(data[data$ks < 0.1, "pi_t_triangle_center"], data[data$ks < 0.1, "ks"])

plot(data[data$ks < 0.1,"T_lat_offset"], data[data$ks < 0.1,"d_symp_lower"])
plot(data[data$ks < 0.1,"T_lat_offset"], data[data$ks < 0.1,"d_symp_upper"])
plot(data[data$ks < 0.1,"T_lat_offset"], data[data$ks < 0.1,"pi_t_triangle_center"])
...
# save.image("~/Dropbox/Ebola/General_Quarantine_Paper/R_Code/20150826_Case_Study_MERS_1a")
...
parms_T_inc = list("lognormal", log(4), log(1.81), 999, "independent", "independent")
names(parms_T_inc) <- c("dist", "parm1", "parm2",  "parm3","anchor_value", "anchor_target")
