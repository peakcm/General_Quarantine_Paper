...
ggplot(data = data.hr.lr[data.hr.lr$R_0 > 2.2 & data.hr.lr$R_0 < 3.6,]) +
  ...
  geom_point(aes(x=R_0, y=R_s, shape = Setting), col = "darkgreen") +
  geom_point(aes(x=R_0, y=R_q, shape = Setting), col = "blue") +
  stat_smooth(aes(x=R_0, y=R_s, shape = Setting), method = "loess", color="darkgreen") +
  stat_smooth(aes(x=R_0, y=R_q, shape = Setting), method = "loess", color="blue") +
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  scale_shape_manual(name = "Setting",
                     values = c(17, 1),
                     labels = c("High Resource", "Low Resource")) +
